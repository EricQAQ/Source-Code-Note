...
int main(int argc, char *argv[])
{
     ...
     * getopt.h的函数被用来解析命令行选项参数
     *
     * int getopt(int argc, char * const argv[], const char *optstring);
     *      argc和argv从main函数传入,optstring表示所有指令,比如:
     *      char *optstring = "abcd:";
     *      ":" 表示参数d可以带参数, 例如-d 100, 参数100可以通过 全局变量optarg获得
     *      getopt函数需要多次调用,直到返回值为-1(EOF)
     *
     * int getopt_long(int argc, char * const argv[], const char *optstring,
                       const struct option *longopts, int *longindex);
            对比getopt函数,多了两个参数: longopts, longindex
            longopts: 指向option结构体组成的数组, 结构体option在getopt.h中定义
     */
    while((opt=getopt_long(argc,argv,"912Vfrt:p:c:?h",long_options,&options_index))!=EOF )
    {
        switch(opt)
        {
            ...
            case 't': benchtime=atoi(optarg);break;	  // 函数atoi用来提取int值, 参数为地址
            case 'p':
                /* proxy server parsing server:port */
                tmp=strrchr(optarg,':'); // 这时optarg是server:Port, tmp= ":port"
                proxyhost=optarg;
                if(tmp==NULL)
                {
                    break;
                }
                if(tmp==optarg) // 缺少hostname
                {
    ...
    // optind 返回第一个不包含选项的命令行参数，此处为URL值(URL为必须参数)
    if(optind==argc)
    {
        // 未填写URL
...
void build_request(const char *url)
{
    ...
    bzero(host,MAXHOSTNAMELEN);     // memset(host, 0, MAXHOSTNAMELEN)
    bzero(request,REQUEST_SIZE);    // memset(request, 0, REQUEST_SIZE)
    ...
    // strstr(const *char, const *char) 找到第一个出现子串的位置, 本例中返回':'的地址
    if(NULL==strstr(url,"://"))
    {
    ...
    if(proxyhost==NULL)
        // strncasecmp 比较前n个字符.未使用代理服务器的情况下，只允许HTTP协议
    ...
    // 两个地址相减,获得':'的相对地址(基于url首字母的地址),+3表示后移3位.指向"://"后的第一个字母
    i=strstr(url,"://")-url+3;
    ...
    if(proxyhost==NULL)
    {
        ...
        {
            // 获取主机地址
            strncpy(host,url+i,strchr(url+i,':')-url-i);    // index()把'\0'当做字符串的一部分, strchr相反
            bzero(tmp,10);
            // 获取端口号地址
    ...
    //完成如 GET /xxx HTTP1.1 后，添加"\r\n"
    strcat(request,"\r\n");
    /*
     * 从本函数开始到现在, request="GET /xxx HTTP1.1\r\n"
     */
